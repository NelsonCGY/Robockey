...
#define threshold_stalker 350
#define threshold_rev 0.4
//#define compensator_uturn 600

#define Kp 0.5    // Red 0.05 Bule 0.025
#define Kd 0.025
...
void stalker() {
    ...
    if ((ADC_V[1] + ADC_V[2]) > (ADC_V[3] + ADC_V[4])) {
        if (diff_12 > 0) {  // turn right
            OCR1C = threshold_rev * OCR1A + Kp_stalker1 * diff_12 - Kd_stalker1 * (diff_12 - diff_12_pre);
            OCR1B = OCR1C;
            set(PORTB, 2);
            clear(PORTB, 3);
        } else {            // turn left
            ...
            case 1:
                ...
                if (diff_12 > 0) {
                    if (diff_12 < threshold_com1) {              // turn left for compensation
                        OCR1C = OCR1A - Kp_com1 * (threshold_com1 - diff_12) - Kd_com1 * (diff_12 - diff_12_pre);
                        OCR1B = OCR1A;
                        set(PORTB, 2);// left
                        set(PORTB, 3);  // right
                        m_usb_tx_string("Left Forward:  ");
                        ...
                        OCR1C = OCR1A;
                        OCR1B = OCR1A - Kp_com2 * (diff_12 - threshold_com2) + Kd_com2 * (diff_12 - diff_12_pre);
                        set(PORTB, 2);	// left
                        set(PORTB, 3);	// right
                        m_usb_tx_string("Left Forward:  ");
                        m_usb_tx_int(OCR1C);
                    ...
                    if (diff_12 > - threshold_com1) {            // turn right for compensation
                        OCR1C = OCR1A;
                        OCR1B = OCR1A - Kp_com1 * (threshold_com1 + diff_12) + Kd_com1 * (diff_12 - diff_12_pre);
                        set(PORTB, 2);  // left
                        set(PORTB, 3);// right
                        m_usb_tx_string("Left Forward:  ");
                        m_usb_tx_int(OCR1C);
                        ...
                        OCR1C = OCR1A + Kp_com2 * (threshold_com2 + diff_12) - Kd_com2 * (diff_12 - diff_12_pre);
                        OCR1B = OCR1A;
                        set(PORTB, 2);	// left
                        set(PORTB, 3);	// right
                        m_usb_tx_string("Left Forward:  ");
