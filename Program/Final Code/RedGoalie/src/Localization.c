...
void go_for_goal(int * goal) {
    ...
    if (sin_ori_goal < 0)
    {
        if ((cos_ori_goal < 0) || (sin_ori_goal < -0.712))	// Right U-turn
        {
            ...
            OCR1B = 0 * OCR1A;    // Red 0.5  Blue 0.4
            set(PORTB, 2);
            clear(PORTB, 3);
...
//                }
        ...
        {
            OCR1B = 0.8 * OCR1A;
            OCR1C = 0.8 * OCR1A - Kp * sin_ori_goal - Kd * (sin_ori_goal - sin_ori_goal_old);
            set(PORTB, 2);		// only move forward
            set(PORTB, 3);
        }
    ...
    else
    {
        ...
        else
        {
            if (dist_robo_goal > threshold_distance_slow) {
                ...
                OCR1B =  0.8 / threshold_distance_slow * dist_robo_goal * OCR1A;
